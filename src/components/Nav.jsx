...
export default function Nav({ setPage, currentPage }) {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-white/95 backdrop-blur-md border-b border-pink-100">
      <div className="max-w-7xl mx-auto px-4 md:px-10 py-3 md:py-4 flex justify-between items-center">
        
        {/* Brand Section */}
        <button onClick={() => setPage('Home')} className="text-left outline-none">
          <h1 className="text-sm md:text-lg font-black uppercase tracking-tighter">
            Star <span className="text-[#b76e79]">Himalayan</span>
          </h1>
          <p className="text-[7px] md:text-[8px] font-bold tracking-[0.2em] uppercase text-gray-400">Export HQ</p>
        </button>
          ...
          {menuItems.map((item) => (
            <button 
              ...
              className={`text-[9px] md:text-[10px] font-bold uppercase tracking-widest transition-all relative py-1 ${
                currentPage === item ? 'text-[#b76e79]' : 'text-gray-500 hover:text-[#b76e79]'
              }`}
            >
              {item}
              {currentPage === item && (
